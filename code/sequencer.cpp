	...
	for(int i = 0; i < CELL_GRID_NUM_H; i++){
		for(Node* n = row[i].next; n != NULL; n = n->next) {
			if(last_played_grid_col <= current_col) {
                ...
                if(n->end   > last_played_grid_col && n->end   <= current_col) {
                    stop_note(i);
                }
			}
			else {
                if(n->start <= current_col || n->start > last_played_grid_col) {
				    play_note(i);
                }
                if(n->end   <= current_col || n->end   > last_played_grid_col) {
                    stop_note(i);
                }
			}
		}
	}
	last_played_grid_col = current_col;
}
